...
import { mintWithToken3, getTokenBalance, getMintWithToken3Price, getMintWithToken3Supply } from "utils/contracts";

function MintWithToken3(props) {
	...
	useEffect(() => {
		if (account && library) {
            const currency = MINT_CONTRACTS.MintWithToken3.currency;
            ...
            getMintWithToken3Price(library.getSigner())
				.then((cost) => {
					setPrice(cost)
				})
				.catch(() => {
            ...
            getMintWithToken3Supply(library.getSigner())
				.then((totalSupply) => {
	...
	const onMintItem = () => {
		if (account && !mintingNFT) {
			...
			setMintingNFT(true);
			mintWithToken3(account, price, numToken, library.getSigner())
				.then((result) => {
					if (result) {
						setMintingNFT(false);
                ...
                <Element.MintCard>
                    ...
                    <div>
                        <Element.ContractDiv>
                            ...
                            <Element.ContractLink href={`https://explorer.kardiachain.io/address/${MINT_CONTRACTS.MintWithToken3.address}`} rel="noopener noreferrer" target="_blank">{MINT_CONTRACTS.MintWithToken3.address}</Element.ContractLink>
                        </Element.ContractDiv>
                    </div>
                </Element.MintCard>
            }
